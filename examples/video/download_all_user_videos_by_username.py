...
def example(username: str, dest_folder: str):
    """
    Download all user videos to folder
    """
    api = TokApi(API_KEY, BASE_URL)

    dest_folder = os.path.join(dest_folder, username)
    if not os.path.exists(dest_folder):
        os.makedirs(dest_folder)

    offset = 0
    has_more = 1
    print('========== START DOWNLOADING VIDEOS FOR USER: {} ========== '.format(username))

    videos = []
    while has_more == 1:
        result = api.get_user_videos_by_different_ids(username=username, with_pinned_posts=1, offset=offset)
...
if __name__ == "__main__":
    example('@rindo.tiktok', os.path.dirname(os.path.abspath(__file__)))
